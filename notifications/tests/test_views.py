...
class ListNotificationsViewTest(TestCase):
    ...
    def setUp(self):
        ...
        self.issue = Issue.objects.create(
            title='Test Issue', reporter=self.user, project=self.project
        )
        ...
        self.change1 = Change.objects.create(
            issue=self.issue, user=self.other_user, field_changed='status',
            old_value='open', new_value='closed',
            changed_at=timezone.now() - timezone.timedelta(days=1)
        )
        self.change2 = Change.objects.create(
            issue=self.issue, user=self.other_user, field_changed='priority',
            old_value='low', new_value='high',
            changed_at=timezone.now() - timezone.timedelta(hours=1)
        )
    ...
    def test_last_visited_notifications_updated(self):
        """
        Test that the last visited notifications timestamp is updated after
...
class ChangeHistoryViewTest(TestCase):
    ...
    def test_change_history_view_issue(self):
        ...
        response = self.client.get(
            reverse('issue_change_history', args=[self.issue1.id])
        )
        self.assertEqual(response.status_code, 200)
        ...
        self.assertGreater(
            response.context['changes'][0].changed_at,
            response.context['changes'][1].changed_at
        )

    def test_change_history_view_no_changes(self):
        """
        Test the change history view when there are no changes for a specific
        issue.
        """
        response = self.client.get(
            reverse('issue_change_history', args=[self.issue2.id])
        )
        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(response, 'change_history.html')
