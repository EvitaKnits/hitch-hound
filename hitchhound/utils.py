"""
This module provides utility functions for the Hitchhound project.

It includes:
- `paginate`: A function to paginate querysets.
- `get_new_notifications`: A function that checks for new notifications
...
def get_new_notifications(user):
    ...
    The function checks for new changes related to issues the user is assigned
    to or has reported,excluding changes made by the user, since the user's
    last visit to the notifications page.
    """
    # Get last time user visited notifications, use current time if not visited
    last_visited = user.last_visited_notifications or timezone.now()

    # Fetch issues the user is assigned to
    user_issues = (
        UserIssue.objects.filter(user=user)
